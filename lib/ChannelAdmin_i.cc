...
ConsumerAdmin_i::remove_proxy(RDI_LocksHeld& held, SequenceProxyPushSupplier_i* prx)
{
    ...
    {
        RDIDbgSPxyLog("Invalid proxy [" << prx->_proxy_id() << "] provided\n");
    }
}
...
ConsumerAdmin_i::_removed_push_proxy(RDIProxyPushSupplier* proxy)
{
  ...
  if ( cpc ) {       // Deregister Proxy
    cpc->remove_proxySupplier(proxy);
  }
}
				...
				RDI_TypeMap*          tmap)
{
  ...
  for (apushcur = _prx_any_push.cursor(); apushcur.is_valid(); ++apushcur ) {
    ...
    tmap->lookupFilter(dname, tname, apushcur.val(), flist);
  ...
  for (apullcur = _prx_any_pull.cursor(); apullcur.is_valid(); ++apullcur ) {
    ...
    tmap->lookupFilter(dname, tname, apullcur.val(), flist);
    if ( ! flist._star_star && ! flist._domn_star &&
	 ! flist._star_type && ! flist._domn_type ) {
      if ( astat == OrMatch ) {
	apullcur.val()->add_event(event);
  ...
  for ( spushcur=_prx_struc_push.cursor(); spushcur.is_valid(); ++spushcur ) {
    ...
    tmap->lookupFilter(dname, tname, spushcur.val(), flist);
    if ( ! flist._star_star && ! flist._domn_star &&
	 ! flist._star_type && ! flist._domn_type ) {
      if ( astat == OrMatch ) {
	spushcur.val()->add_event(event);
  ...
  for ( spullcur=_prx_struc_pull.cursor(); spullcur.is_valid(); ++spullcur ) {
    ...
    tmap->lookupFilter(dname, tname, spullcur.val(), flist);
    if ( ! flist._star_star && ! flist._domn_star &&
	 ! flist._star_type && ! flist._domn_type ) {
      if ( astat == OrMatch ) {
	spullcur.val()->add_event(event);
  ...
  for ( bpullcur=_prx_batch_pull.cursor(); bpullcur.is_valid(); ++bpullcur ) {
    ...
    tmap->lookupFilter(dname, tname, bpullcur.val(), flist);
    if ( ! flist._star_star && ! flist._domn_star &&
	 ! flist._star_type && ! flist._domn_type ) {
      if ( astat == OrMatch ) {
	bpullcur.val()->add_event(event);
  ...
  RDIDbgCosCPxyLog("Thrd=" << TW_ID() << ", Channel=" << _channel->MyID() << ", ConsumerAdmin_i::dispatch_event - end"
      << ", event_queue=" << reinterpret_cast<EventChannel_i_stub*>(_channel)->_events->length()
      << ", proxy_queue=" << reinterpret_cast<EventChannel_i_stub*>(_channel)->_proxy_events.length()
      << ", time_elapsed=" << ThreadTimeStamp::instance().get_elapse_in_nanosecond( "ConsumerAdmin_i::dispatch_event" ) << "n"
      << "\n");
#endif
}
