...
StructuredProxyPushSupplier_i::push_event(CORBA::Boolean& invalid)
{
	...
	// Do not hold OPLOCK across push
	outcall_worked = 0;
	// attempt to retry the call three times
	int retryCount(0);
	const char* exceptionType;
	{
		...
		while ( !outcall_worked && ( retryCount < 3 ) ) 
		{ 
			// introduce unlock scope
			...
			{
				// Get the exception minor string for printing out
				exceptionType = e.NP_minorString();
				RDIDbgSPxyLog("Push pool calling push_event for proxy " << (void*)this << " - " << exceptionType << "\n" );
			...
			catch ( CORBA::TRANSIENT& e ) 
			{
				// Get the exception minor string for printing out
				exceptionType = e.NP_minorString();			
				RDIDbgSPxyLog("Push pool calling push_event for proxy " << (void*)this << " - " << 
					exceptionType << ", trying again. Retry count = " << retryCount << "\n");

				// don't break out so we can try again
			}
			catch ( CORBA::INV_OBJREF& e ) 
			{
				// Get the exception minor string for printing out
			...
			{
				RDIDbgSPxyLog("Push pool calling push_event for proxy " << (void*)this << " - Unknown Exception while pushing event to consumer\n");			
				// Break out of the retry loop
				break;
			}
			//RDI_SEVENT_DECR_REF_COUNTER(event, WHATFN);//--RenQiang
		}//end while
	}// end unlock scope

	RDI_SEVENT_DECR_REF_COUNTER(event, WHATFN);//++RenQiang
	RDIDbgSPxyLog("StructuredProxyPushSupplier_i::push_event called\n");//++RenQiang			
	...
	else 
	{
		...
		{ 
			// exception
			// unregister this proxy -- no more subscription_change msgs
			RDIDbgSPxyLog("StructuredProxyPushSupplier " << (void*)this << 
				" in exception, unregistering and removing. Attempts: " << retryCount << "\n");
...
StructuredProxyPushSupplier_i::_push_event()
{
	...
	while ( 1 ) 
	{
		...
		RDI_SEVENT_DECR_REF_COUNTER(event, WHATFN);//++RenQiang
		RDIDbgSPxyLog("StructuredProxyPushSupplier_i::_push_event called\n");//++RenQiang			
...
SequenceProxyPushSupplier_i::push_event(CORBA::Boolean& invalid)
{
	...
	{
        RDIDbgForceLog("SequenceProxyPushSupplier_i::_push_event failed, proxystate: " << _pxstate << '\n');
		return;
	}
	...
	if ( qsize < bsize) 
	{ 
		// one/both of pacing_interval_s/_n must be non-zero
		TW_GET_TIME(&time_s, &time_n);

		if ((_timeout_s == 0) && (_timeout_n == 0)) 
		{ // timeout not set; set it now
			TW_GET_TIME(&_timeout_s, &_timeout_n, pacing_interval_s, pacing_interval_n);
		} 
		if ( (time_s < _timeout_s) || ((time_s == _timeout_s) && (time_n < _timeout_n)) )
		{
			RDIDbgForceLog("** INTERNAL ERROR: SequenceProxyPushSupplier_i::push_event called too soon\n");
			return;
		}
	}
	...
	const char* exceptionType;
	{
        ...
        RDI_TimeT curtime;
        RDI_TimeT outcallBeginTime;
        outcallBeginTime.set_curtime();
        CORBA::ULong deadConProxyInterval = RDI::get_server_qos()->deadConProxyInterval;
        ...
        do
        {
	        ...
	        catch ( CORBA::COMM_FAILURE& e ) 
	        {
		        // Get the exception minor string for printing out
		        exceptionType = e.NP_minorString();			
		        RDIDbgForceLog("Push pool calling push_event for proxy " << (void*)this << " - " << exceptionType << "\n" );
	        ...
	        catch ( CORBA::TRANSIENT& e ) 
	        {
		        // Get the exception minor string for printing out
		        exceptionType = e.NP_minorString();			
		        RDIDbgForceLog("Push pool calling push_event for proxy " << (void*)this << " - " << 
			        exceptionType << ", trying again. Retry count = " << retryCount << "\n");
	        ...
	        {
		        RDIDbgForceLog("Push pool calling push_event for proxy " << (void*)this << " - Unknown Exception while pushing event to consumer\n");			
		        // Break out of the retry loop
		        // break;
	        }
            ...
            TW_SLEEP(1);
            curtime.set_curtime();
        } while ( ( _pxstate == RDI_Connected && ( RDI_TIMET_DIFF_IN_SECS(_last_use, curtime) < deadConProxyInterval ) && ( RDI_TIMET_DIFF_IN_SECS(outcallBeginTime, curtime) < 3 * deadConProxyInterval ) ) );
	}// end unlock scope
	...
	if (_pxstate != RDI_Connected) 
	{
        RDIDbgForceLog("SequenceProxyPushSupplier_i::_push_event do nothing, proxystate: " << _pxstate << '\n');
		// disconnected/disposed during push -- do nothing else with my state
	}
	else
	{
		...
		else
		{
			// exception
			// unregister this proxy -- no more subscription_change msgs
			RDIDbgForceLog("SequenceProxyPushSupplier " << (void*)this << 
				" in exception, unregistering and removing. Attempts: " << retryCount << ", will clear the event queue\n");
...
SequenceProxyPushSupplier_i::_push_event()
{
	...
	if (!held.sproxy) 
	{ 
        RDIDbgForceLog("SequenceProxyPushSupplier_i::_push_event failed, could not held proxy lock \n");
		return;
	}
	...
	while ( 1 ) 
	{
		...
		else 
		{
			...
			else 
			{
				// exception
				// unregister this proxy -- no more subscription_change msgs
				RDIDbgSPxyLog("SequenceProxyPushSupplier " << (void*)this << 
					" in exception, unregistering and removing. Attempts: " << retryCount << "\n");
