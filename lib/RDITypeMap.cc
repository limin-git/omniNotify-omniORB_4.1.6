		    ...
		    Filter_i*                  filter)
{
  ...
  for ( indx = 0; indx < deled.length(); indx++ ) {
    ...
    if ( ! value._admn && ! value._prxy ) {
	  ...
	  // EDIT! - use EventType operator=
      gdeled[size] = deled[indx];
		    ...
		    Filter_i*                  filter)
{
    ...
    CORBA::ULong size;
    unsigned int indx;
    PNode_t *currProxyNode, *prevProxyNode;
    FNode_t *currFilterNode, *prevFilterNode;
  ...
  gdeled.length(0);
    ...
    for ( indx = 0; indx < deled.length(); indx++ ) 
    {
        if ( _tmap.lookup(deled[indx], matchedNode) == 0 )
            continue;
        currProxyNode = matchedNode._prxy; 
        prevProxyNode = 0;	
        // Locate proxy
        while ( currProxyNode && currProxyNode->_prxy != proxy ) 
        {
            prevProxyNode = currProxyNode;
            currProxyNode = currProxyNode->_next;
        }
        if ( ! currProxyNode )
            continue;

        currFilterNode = currProxyNode->_fltr;
        prevFilterNode = 0;
        while ( currFilterNode && (currFilterNode->_fltr != filter) ) 
        {
            prevFilterNode = currFilterNode;
            currFilterNode = currFilterNode->_next;
        }
        if ( ! currFilterNode )
            continue;
        
        // Remove filter entry
        if ( prevFilterNode == NULL )
        {
            currProxyNode->_fltr = currFilterNode->_next;
        }
        else
        {
            prevFilterNode->_next = currFilterNode->_next;
        }
        delete currFilterNode;

        if ( ! currProxyNode->_fltr ) 
        {		
            // Remove proxy entry
            if ( NULL  == prevProxyNode ) 
            {
                matchedNode._prxy = currProxyNode->_next;
                _tmap.replace(deled[indx], matchedNode);
            } 
            else 
            {
                prevProxyNode->_next = currProxyNode->_next;
            }
            delete currProxyNode;
        }

        if ( ! matchedNode._admn && ! matchedNode._prxy ) 
        {
            size = gdeled.length(); gdeled.length(size + 1);
    ...
    for ( indx = 0; indx < added.length(); indx++ )
    {
        currFilterNode = new FNode_t(filter);

        if ( _tmap.lookup(added[indx], matchedNode) == 0 ) 
        {
            currProxyNode = new PNode_t(proxy, currFilterNode);
            matchedNode._prxy = currProxyNode;
            matchedNode._admn = 0;
            if ( _tmap.insert(added[indx], matchedNode) ) 
            {
                delete currFilterNode;
                return 0;
            }
        ...
        {
            for ( currProxyNode = matchedNode._prxy; currProxyNode; currProxyNode = currProxyNode->_next ) 
            {
                if ( currProxyNode->_prxy == proxy )
                    break;
            }
            if ( ! currProxyNode ) 
            {
                ...
                currProxyNode->_next = matchedNode._prxy;
                matchedNode._prxy  = currProxyNode;
                _tmap.replace(added[indx], matchedNode);
            } 
            else 
            {
                currFilterNode->_next = currProxyNode->_fltr;
                currProxyNode->_fltr = currFilterNode;
            }
        }
    }
    // Notify suppliers about changes in the event types, if any
...
void
RDI_TypeMap::lookupFilter(const char* dname, const char*    tname,
		    ConsumerAdmin_i*                  admin, 
...
void
RDI_TypeMap::lookupFilter(const char*                      dname,
		    const char*                      tname,
		    RDIProxySupplier*                proxy,
		    RDI_TypeMap::FList_t&            filters)
{
