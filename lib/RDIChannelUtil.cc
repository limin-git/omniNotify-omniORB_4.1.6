...
void
RDI_NotifyConsumer::insert_proxySupplier(RDIProxyPushSupplier* proxy)
{
...
void
RDI_NotifyConsumer::remove_proxySupplier(RDIProxyPushSupplier* proxy)
{
...
RDI_NotifyConsumer::notify()
{
  ...
  unsigned long wait_s = 0, wait_n = 0;
    ...
    while ( 1 ) 
    {
        { // introduce scope lock
            TW_SCOPE_LOCK(notifcon_lock, _oplock, "notifcon", WHATFN);
            while ( (! _terminate) && ( NULL == (node = _next_available(&wait_s, &wait_n))) ) 
            {
                if ( (wait_s > 0) || (wait_n > 0) ) 
                {
...
RDI_NotifyConsumer::_next_available(unsigned long* wait_s, unsigned long* wait_t)
{
    ...
    while ( numIter < 3 ) 
    {
        if ( (! _mylast_one) || (! _mylast_one->_next) ) 
        {
            numIter += 1;
            _mylast_one = _entries;
        } 
        else 
        {
            _mylast_one = _mylast_one->_next;
        }
        if ( (! _mylast_one->_inuse) && 
            (! _mylast_one->_deled) &&
            _mylast_one->_proxy->has_events(wait_s, wait_t) ) 
        {
            RDIDbgChanLog("NotifyConsumer::_next_available -- found entry for proxy " << _mylast_one->_proxy << '\n');
            return _mylast_one;
        }
    }
...
RDI_ChangePool::RDI_ChangePool(RDI_ChangeKind kind, unsigned int numThreads) :
  ...
  _validProxyCount(0), _deletedProxyCount(0), _nactive(0), _terminate(0), 
  _c_head(0), _c_tail(0), _centryCount(0)
{
  { // introduce lock scope so that release occurs before constructor finishes
...
RDI_ChangePool::destroy()
{
  ...
  while ( _c_head ) {
    ...
    ProxyEntry_t* node = _c_head->_mypxy_head;
    while (node) {
			      ...
			      const CosN::EventTypeSeq& deled)
{
  ...
  { // introduce lock scope
    TW_SCOPE_LOCK(changepool_lock, _oplock, "changepool", WHATFN);
    if ( _terminate || (_validProxyCount == 0) ) {
    ...
    //remove node don't link to proxy node, since it won't notify subscriber
    if ( _centryCount > 10)
    {
        deleteEmptyEntryNode();
		_centryCount = 0;
    }
...
void
RDI_ChangePool::insert_proxyConsumer(RDIProxyConsumer* proxy)
{
    ...
    { // introduce lock scope
        ...
        if (_c_tail->_mypxy_head == (ProxyEntry_t*)0) 
        {
            _c_tail->_mypxy_head = node;
        }
        node->_pxy_next = 0;
        _validProxyCount++;
        //RDIDbgForceLog("ProxyConsumer proxy " << proxy << " will get subscription_change msgs, _nproxies = " << _validProxyCount << '\n');
    } // end lock scope
}
...
RDI_ChangePool::insert_proxy(RDIProxySupplier* proxy)
{
  ...
  { // introduce lock scope
    ...
    node->_pxy_next = 0;
    _validProxyCount++;
    RDIDbgChanLog("ProxySuplier proxy " << proxy << " will get offer_change msgs, _nproxies = " << _validProxyCount << '\n');
  } // end lock scope
}
...
// or if the provided reference is NIL.
void
RDI_ChangePool::remove_proxyConsumer(RDIProxyConsumer* proxy)
{
    ...
    { // introduce lock scope
        ...
        {
            for ( node = entry->_mypxy_head; node != (ProxyEntry_t *) 0; node = node->_pxy_next ) 
            {
...
{
    CORBA::Boolean     invalid;
    ProxyEntry_t*      currProxyEntryNode        = 0; // next node to work on
    ...
    while ( 1 ) 
    {
        { // introduce lock scope
            TW_SCOPE_LOCK(changepool_lock, _oplock, "changepool", WHATFN);
            while ( (! _terminate) && 
                ( (_validProxyCount == 0) || (! (currProxyEntryNode = _next_available(&found_entry))) ) ) 
            {
        ...
        invalid = 0;
        if ( (! currProxyEntryNode->_deled) ) 
        {
            if (_kind == RDI_ChangeKind_SubscriptionChange) 
            {
                invalid = currProxyEntryNode->_cproxy->send_subscription_change(found_entry->_added, found_entry->_deled);
                /*
                RDIDbgForceLog("Sending subscription_change to ProxyConsumer proxy, CEntry: " << found_entry
                <<" totalProxyCount: " <<  _validProxyCount
                <<" currentCEntryCount: " << _centryCount
                << ", ProcessEntry: "  << currProxyEntryNode 
                << ", ProxyConsumer: " << currProxyEntryNode->_cproxy << ", invalid: " << invalid << '\n');
                */

            } 
            else 
            {
                RDIDbgChanLog("Sending offer_change to ProxySupplier proxy " << currProxyEntryNode->_sproxy << '\n');
                invalid = currProxyEntryNode->_sproxy->send_offer_change(found_entry->_added, found_entry->_deled);
            }
        }
    ...
    { // introduce lock scope
      TW_SCOPE_LOCK(changepool_lock, _oplock, "changepool", WHATFN);
            ...
            found_entry->_inuseProxyCount--;
            currProxyEntryNode->_inuse = 0;
            ...
            if ( invalid ) 
            {
                if ( (! currProxyEntryNode->_deled) ) 
                {
                    ...
                    currProxyEntryNode->_deled = 1;
                    _validProxyCount--;
                    _deletedProxyCount++;
                    if (_kind == RDI_ChangeKind_SubscriptionChange) 
                    {
                        RDIDbgChanLog("ProxyConsumer proxy " << currProxyEntryNode->_cproxy << " will no longer get subscription_change msgs, _nproxies = " << _validProxyCount << '\n');
                    } 
                    else 
                    {
                        RDIDbgChanLog("ProxySupplier proxy " << currProxyEntryNode->_sproxy << " will no longer get offer_change msgs, _nproxies = " << _validProxyCount << '\n');
                    }
                    if (_deletedProxyCount >= 10) 
                    {
                        _poolGCollect();
            ...
            { // give another thread a shot at choosing this valid node
                _nonempty.signal();
            }
        } // end lock scope
        TW_YIELD();
...
RDI_ChangePool::_next_available(CEntry_t** found_entry)
{
    ...
    {
        ProxyEntry_t* currProxyEntry = c_entry->_mypxy_head;
        ProxyEntry_t* prevProxyEntry = 0;
        while (currProxyEntry != (ProxyEntry_t *) 0) 
        {
            if ( currProxyEntry->_deled ) 
            {
                RDIDbgChanLog("RDI_ChangePool::_next_available: skipping deleted node\n");
            } 
            else if ( currProxyEntry->_inuse ) 
            {
            ...
            else 
            {
                ...
                (*found_entry) = c_entry;
                CEntry_t* skipEntry = c_entry->_c_skip;
                c_entry->_inuseProxyCount++;
                currProxyEntry->_inuse = 1;
                // unlink node
                if (prevProxyEntry) 
                {
                    RDIDbgChanLog("Removing proxy " << currProxyEntry << " from prev_node " <<
                        prevProxyEntry << " of CEntry " << c_entry << '\n');
                    prevProxyEntry->_pxy_next = currProxyEntry->_pxy_next;
                } 
                else 
                {
                    RDIDbgChanLog("Removing proxy " << currProxyEntry << " from head of CEntry " <<
                        c_entry << " proxy chain\n");
                    c_entry->_mypxy_head = currProxyEntry->_pxy_next;
                }
                // adjust _pxy_tail if necessary
                if (c_entry->_pxy_tail == currProxyEntry) 
                {
                    RDIDbgChanLog("Adjusting _pxy_tail of CEntry " << c_entry << " new val = " << prevProxyEntry << '\n');
                    c_entry->_pxy_tail = prevProxyEntry;
                }
                
                // append node to (*found_entry)->_c_skip proxy list
                // RDIDbgChanLog("Adding proxy " << currProxyEntry << " to tail of CEntry " << c_entry << " ->_c_skip "  << skipEntry << " proxy list\n");
                if (skipEntry->_pxy_tail) 
                {
                    skipEntry->_pxy_tail->_pxy_next = currProxyEntry;
                }
                skipEntry->_pxy_tail = currProxyEntry;
                if (skipEntry->_mypxy_head == (ProxyEntry_t*)0) 
                {
                    skipEntry->_mypxy_head = currProxyEntry;
                }
                
                currProxyEntry->_pxy_next = 0;
                RDIDbgChanLog("Returning result = " << currProxyEntry << '\n');
                return currProxyEntry; // success
            }
            // advance p_entry
            prevProxyEntry = currProxyEntry;
            currProxyEntry = currProxyEntry->_pxy_next;
        }
    }
    // failed for now
...
void
RDI_ChangePool::_poolGCollect() 
{

    RDIDbgChanLog("RDI_ChangPool::_gcollect called with _ndeleted == " << _deletedProxyCount << '\n'); 
    ...
    for (CEntry_t* c_ptr = _c_head; c_ptr != (CEntry_t*)0; c_ptr = c_ptr->_c_next) 
    {
        ...
        while ( (p_ptr = c_ptr->_mypxy_head) && (! p_ptr->_inuse) && p_ptr->_deled ) 
        {
            c_ptr->_mypxy_head = p_ptr->_pxy_next; // unlinks p_ptr
            RDIDbgChanLog("RDI_ChangePool::_gcollect: discarding deleted proxy entry " << p_ptr << '\n');
            delete p_ptr;
            _deletedProxyCount--;
        }
        if ( (p_ptr = c_ptr->_mypxy_head) ) 
        {
        ...
        { // nothing left in proxy chain, fix _pxy_tail
            c_ptr->_pxy_tail = 0;
        }
    }
    ...
    RDIDbgChanLog("RDI_ChangePool::_gcollect: after GC, _ndeleted == " << _deletedProxyCount << '\n');
}

//assume the data has been locked before
void RDI_ChangePool::deleteEmptyEntryNode()
{
    //first delete the head without no proxy.
    deleteHeadNoUseNode();

    /*
    CEntry_t * prevEntryNode = 0;
    //skip the 
    CEntry_t * currEntryNode = _c_head->_c_next;
    for ( ; currEntryNode != _c_tail; currEntryNode = c_entry->_c_next)
    {
        if ( currEntryNode
    }
    */
...
void RDI_ChangePool::deleteHeadNoUseNode()
{
    while ( _c_head->_valid && (! _c_head->_mypxy_head) &&
        _c_head->_c_next && (_c_head->_inuseProxyCount == 0) ) 
    {
        CEntry_t* c_tmp = _c_head;
        _c_head = _c_head->_c_next;
        //RDIDbgForceLog("RDI_ChangePool::_gcollect: discarding unnecessary _c_head: " << c_tmp << '\n');
        delete c_tmp;
        _centryCount--;
    }
}
