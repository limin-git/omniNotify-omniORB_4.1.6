...
RDIProxyConsumer::_enable_updates()
{
  ...
  if (!_sc_off) {
    ...
    cscp->remove_proxyConsumer(this);
  }
  _sc_off = 0;
  cscp->insert_proxyConsumer(this);
}
		...
		const char* exceptionType;
		{
			...
			{ 
				...
				catch( CORBA::NO_IMPLEMENT& e ) 
				{
					...
					RDIDbgCPxyLog("RDIProxyConsumer subscription_change failed - " << exceptionType << "\n");

					res = 1; // do not try any more of these
					// Do not try the call again
					break;
				}
				...
				catch ( CORBA::TRANSIENT& e ) 
				{
					// Get the exception minor string for printing out
					exceptionType = e.NP_minorString();			
					RDIDbgCPxyLog("RDIProxyConsumer subscription_change failed - " << 
						exceptionType << ", trying again. Retry count = " << retryCount << "\n");

					// don't break out so we can try again, flag the failure anyway
					res = 1;
				}
				catch (...) 
				{
					RDIDbgCPxyLog("RDIProxyConsumer subscription_change failed due to an unknown exception\n");
...
SequenceProxyPushConsumer_i::log_output(RDIstrstream& str) const
{
  str.setw(8); str << (void*)this;
    ...
    str << " -- ADDRESS "; str.setw(21); str << ObjectAddress::get_object_address_str( _supplier ).c_str();
#endif
