...
public:
  ...
  void insert_proxySupplier(RDIProxyPushSupplier* proxy);
  void remove_proxySupplier(RDIProxyPushSupplier* proxy);
...
private:
  ...
  CORBA::Boolean          _terminate;    // Set to true  during destruction
  ProxyEntry_t*           _entries;      // Registered  push  ProxySupplier
  ProxyEntry_t*           _mylast_one;     // Last entry accessed by a thread
...
public:
  ...
  void insert_proxyConsumer(RDIProxyConsumer* proxy);
  void remove_proxyConsumer(RDIProxyConsumer* proxy);
  ...
  struct CEntry_t {
    ...
    unsigned int        _inuseProxyCount;    // Number of nodes on proxy chain in use
    CosN::EventTypeSeq   _added;
    CosN::EventTypeSeq   _deled;
    ProxyEntry_t*       _mypxy_head;     // Head of chain of ProxyEntry
    ...
    CEntry_t() :
      _valid(0), _inuseProxyCount(0), _added(), _deled(),
      _mypxy_head(0), _pxy_tail(0), _c_next(0)
      , _c_skip(0)
    {
      _added.length(0);
      _deled.length(0);
...
private:
  ...
  TW_Condition             _nonempty;
  TW_Thread**              _threads;
  unsigned long            _validProxyCount;     // Number of valid (non-deleted) proxy entries
  unsigned long            _deletedProxyCount;     // Number of deleted proxy entries
  unsigned long            _centryCount;
  unsigned int             _nactive;      // Number of threads still running
  CORBA::Boolean           _terminate;    // Set to true  during destruction
  ...
  //delete entry node don't link to proxy node, since it won't notify subscriber, will cause memory leak
  void deleteEmptyEntryNode();

  void deleteHeadNoUseNode();
};
